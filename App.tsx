  ...
  const handleDownloadCode = async () => {
     try {
        ...
        await Promise.all(filesToFetch.map(async (filename) => {
           try {
              // Try to fetch relative to root
              const response = await fetch(`./${filename}`);
              if (response.ok) {
                 const text = await response.text();
                 // If fetching App.tsx returns HTML (SPA fallback), we ignore it
        ...
        if (successCount === 0) {
           showNotification("Could not auto-fetch files. Please copy manually.");
           return;
        }
